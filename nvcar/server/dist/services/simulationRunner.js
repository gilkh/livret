...
exports.runSimulation = exports.stopSimulation = exports.getLiveSimulationState = void 0;
...
const recordAction = async (runId, action) => {
    ...
    await SimulationRun_1.SimulationRun.findByIdAndUpdate(runId, {
        $push: {
            recentActions: {
                $each: [action],
                $slice: -200,
...
const buildDataPatchFromTemplate = (template) => {
    ...
    for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
        const blocks = Array.isArray(pages[pageIdx]?.blocks) ? pages[pageIdx].blocks : [];
...
const createSimUser = async (role, runId) => {
    ...
    const user = await User_1.User.create({ email, passwordHash, role, displayName });
    const token = (0, auth_1.signToken)({ userId: String(user._id), role });
...
const ensureSeededData = async (cfg, teacherIds, subAdminIds) => {
    ...
    const now = new Date();
    let year = await SchoolYear_1.SchoolYear.findOne({ active: true }).lean();
    if (!year) {
        year = await SchoolYear_1.SchoolYear.create({
    ...
    for (let i = 0; i < classCount; i++) {
        ...
        const cls = await Class_1.ClassModel.create({ name: `SIM ${cfg.runId.slice(0, 6)} ${String.fromCharCode(65 + i)}`, level, schoolYearId });
    ...
    for (let i = 0; i < teacherIds.length; i++) {
        ...
        try {
            await TeacherClassAssignment_1.TeacherClassAssignment.create({
                teacherId,
                classId,
                schoolYearId,
                languages: ['FR', 'AR', 'EN'],
    ...
    for (let c = 0; c < classIds.length; c++) {
        ...
        for (let s = 0; s < studentsPerClass; s++) {
            const student = await Student_1.Student.create({
                ...
                logicalKey: `SIM_${cfg.runId.slice(0, 6)}_${(0, crypto_1.randomUUID)().slice(0, 6)}`,
                level,
                schoolYearId,
            });
            ...
            if (classId) {
                ...
                if (studentId) {
                    ...
                    if (assignmentId) {
                        ...
                        const dataPatch = buildDataPatchFromTemplate(tpl);
                        ...
                        if (tpl && Math.random() < 0.4) {
                            ...
                            for (let p = 0; p < pages.length; p++) {
                                const blocks = Array.isArray(pages[p]?.blocks) ? pages[p].blocks : [];
                            ...
                            if (c) {
                                const nextItems = c.items.map((it) => ({ ...it, active: Math.random() < 0.6 }));
...
const computeSummary = (actions, durationSec) => {
...
const getLiveSimulationState = (runId) => {
    return liveByRunId.get(runId) || null;
};
exports.getLiveSimulationState = getLiveSimulationState;
const stopSimulation = async (runId) => {
    const live = liveByRunId.get(runId);
    if (live) {
...
const runSimulation = async (cfg) => {
    ...
    try {
        ...
        if (Array.isArray(seeded.assignmentIds))
            cfg.seededAssignmentIds = seeded.assignmentIds;
        if (Array.isArray(seeded.classIds))
            cfg.seededClassIds = seeded.classIds;
        ...
        const sampler = (async () => {
            while (!live.stopRequested && Date.now() - startedAt < cfg.durationSec * 1000) {
                ...
                await SimulationRun_1.SimulationRun.findByIdAndUpdate(cfg.runId, { $set: { lastMetrics: live.lastMetrics } });
                await sleep(1000);
            }
        })();
        ...
        const collector = (async () => {
            while (!live.stopRequested && Date.now() - startedAt < cfg.durationSec * 1000) {
                ...
                const recent = run?.recentActions || [];
        ...
        try {
            if (createdUserIds.length > 0) {
                await User_1.User.deleteMany({ _id: { $in: createdUserIds } });
            }
        }
        catch (e) {
        }
