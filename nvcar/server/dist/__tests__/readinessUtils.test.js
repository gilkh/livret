...
describe('readinessUtils', () => {
    describe('computeSignaturePeriodId', () => {
        it('generates deterministic period ID for sem1', () => {
            ...
            const result = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'sem1');
            expect(result).toBe('507f1f77bcf86cd799439011_sem1');
        });
        it('generates deterministic period ID for sem2', () => {
            const schoolYearId = '507f1f77bcf86cd799439011';
            const result = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'sem2');
            expect(result).toBe('507f1f77bcf86cd799439011_sem2');
        });
        it('generates deterministic period ID for end_of_year', () => {
            const schoolYearId = '507f1f77bcf86cd799439011';
            const result = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'end_of_year');
            expect(result).toBe('507f1f77bcf86cd799439011_end_of_year');
        });
        it('throws error when schoolYearId is empty', () => {
            expect(() => (0, readinessUtils_1.computeSignaturePeriodId)('', 'sem1')).toThrow('schoolYearId is required');
        });
        it('is deterministic - same inputs produce same output', () => {
            const schoolYearId = '507f1f77bcf86cd799439011';
            const periodType = 'sem1';
            const result1 = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, periodType);
            const result2 = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, periodType);
            const result3 = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, periodType);
            expect(result1).toBe(result2);
            expect(result2).toBe(result3);
        });
        it('produces different IDs for different periods', () => {
            const schoolYearId = '507f1f77bcf86cd799439011';
            const sem1 = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'sem1');
            const sem2 = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'sem2');
            const endOfYear = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, 'end_of_year');
        ...
        it('produces different IDs for different school years', () => {
            const year1 = (0, readinessUtils_1.computeSignaturePeriodId)('year1id', 'sem1');
            const year2 = (0, readinessUtils_1.computeSignaturePeriodId)('year2id', 'sem1');
            expect(year1).not.toBe(year2);
        });
    });
    describe('parseSignaturePeriodId', () => {
        it('parses sem1 period ID correctly', () => {
            const result = (0, readinessUtils_1.parseSignaturePeriodId)('507f1f77bcf86cd799439011_sem1');
        ...
        it('parses sem2 period ID correctly', () => {
            const result = (0, readinessUtils_1.parseSignaturePeriodId)('507f1f77bcf86cd799439011_sem2');
            expect(result).toEqual({
                schoolYearId: '507f1f77bcf86cd799439011',
                periodType: 'sem2'
            });
        ...
        it('parses end_of_year period ID correctly', () => {
            const result = (0, readinessUtils_1.parseSignaturePeriodId)('507f1f77bcf86cd799439011_end_of_year');
            expect(result).toEqual({
                schoolYearId: '507f1f77bcf86cd799439011',
        ...
        it('returns null for empty input', () => {
            expect((0, readinessUtils_1.parseSignaturePeriodId)('')).toBeNull();
        });
        it('returns null for invalid period type', () => {
            expect((0, readinessUtils_1.parseSignaturePeriodId)('507f1f77bcf86cd799439011_invalid')).toBeNull();
        });
        it('handles school year IDs that contain underscores', () => {
            // Edge case: schoolYearId might contain underscores
            const result = (0, readinessUtils_1.parseSignaturePeriodId)('year_with_underscore_sem1');
            expect(result).toEqual({
        ...
        it('roundtrips correctly', () => {
            const schoolYearId = '507f1f77bcf86cd799439011';
            const periodType = 'end_of_year';
            const periodId = (0, readinessUtils_1.computeSignaturePeriodId)(schoolYearId, periodType);
            const parsed = (0, readinessUtils_1.parseSignaturePeriodId)(periodId);
    ...
    describe('sanitizeDataForNewAssignment', () => {
        it('adds copiedFrom metadata', () => {
            ...
            const result = (0, readinessUtils_1.sanitizeDataForNewAssignment)(data, 'source123');
        ...
        it('handles null input', () => {
            const result = (0, readinessUtils_1.sanitizeDataForNewAssignment)(null, 'source123');
            expect(result._copiedFrom).toBeDefined();
            expect(Object.keys(result).length).toBe(1); // Only _copiedFrom
        });
        it('handles undefined input', () => {
            const result = (0, readinessUtils_1.sanitizeDataForNewAssignment)(undefined, 'source123');
            expect(result._copiedFrom).toBeDefined();
        });
    ...
    describe('buildSavedGradebookMeta', () => {
        it('builds meta object with all required fields', () => {
            const result = (0, readinessUtils_1.buildSavedGradebookMeta)({
                templateVersion: 3,
                dataVersion: 5,
    ...
    describe('SAFE_DATA_FIELDS_ALLOWLIST', () => {
        it('contains expected prefixes', () => {
            expect(readinessUtils_1.SAFE_DATA_FIELDS_ALLOWLIST).toContain('language_toggle_');
            expect(readinessUtils_1.SAFE_DATA_FIELDS_ALLOWLIST).toContain('table_');
            expect(readinessUtils_1.SAFE_DATA_FIELDS_ALLOWLIST).toContain('dropdown_');
            expect(readinessUtils_1.SAFE_DATA_FIELDS_ALLOWLIST).toContain('text_');
        });
    });
    describe('SYSTEM_FIELDS_BLOCKLIST', () => {
        it('contains system fields that should never be copied', () => {
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('signatures');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('promotions');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('completed');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('completedSem1');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('completedSem2');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('isCompleted');
            expect(readinessUtils_1.SYSTEM_FIELDS_BLOCKLIST).toContain('status');
        });
    });
});
