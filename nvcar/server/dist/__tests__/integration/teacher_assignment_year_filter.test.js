...
describe('teacher assignment year filtering', () => {
    ...
    it('does not carry over completion when a legacy assignment is stamped into a new year', async () => {
        const prev = await SchoolYear_1.SchoolYear.create({ name: '2025/2026', startDate: new Date('2025-09-01'), endDate: new Date('2026-07-01'), active: false });
        const active = await SchoolYear_1.SchoolYear.create({ name: '2026/2027', startDate: new Date('2026-09-01'), endDate: new Date('2027-07-01'), active: true });
        const clsActive = await Class_1.ClassModel.create({ name: 'ClassActive', level: 'PS', schoolYearId: String(active._id) });
        const admin = await User_1.User.create({ email: 'admin2', role: 'ADMIN', displayName: 'Admin2', passwordHash: 'hash' });
        const sub = await User_1.User.create({ email: 'sub2', role: 'SUBADMIN', displayName: 'Sub2', passwordHash: 'hash' });
        await RoleScope_1.RoleScope.create({ userId: String(sub._id), levels: ['PS'] });
        const t = await User_1.User.create({ email: 't3', role: 'TEACHER', displayName: 'Teacher3', passwordHash: 'hash' });
        await TeacherClassAssignment_1.TeacherClassAssignment.create({ teacherId: String(t._id), classId: String(clsActive._id), schoolYearId: String(active._id), assignedBy: String(admin._id) });
        const tpl = await GradebookTemplate_1.GradebookTemplate.create({ name: 'tpl-legacy', pages: [], currentVersion: 1 });
        const otherStudent = await Student_1.Student.create({ firstName: 'O', lastName: 'S', dateOfBirth: new Date('2018-01-01'), logicalKey: 'S3' });
        await Enrollment_1.Enrollment.create({ studentId: String(otherStudent._id), classId: clsActive._id, schoolYearId: String(active._id), status: 'active' });
        await TemplateAssignment_1.TemplateAssignment.create({ templateId: String(tpl._id), studentId: String(otherStudent._id), completionSchoolYearId: String(active._id), assignedTeachers: [String(t._id)], assignedBy: String(admin._id), status: 'draft', isCompleted: false });
        const student = await Student_1.Student.create({ firstName: 'S', lastName: 'L', dateOfBirth: new Date('2018-01-01'), logicalKey: 'S4' });
        const legacy = await TemplateAssignment_1.TemplateAssignment.create({
            templateId: String(tpl._id),
            studentId: String(student._id),
            assignedTeachers: [String(t._id)],
            assignedBy: String(admin._id),
        ...
        expect(res.status).toBe(200);
        const after = await TemplateAssignment_1.TemplateAssignment.findById(legacy._id).lean();
        expect(after).toBeTruthy();
        expect(String(after.completionSchoolYearId)).toBe(String(active._id));
        expect(after.isCompleted).toBe(false);
        expect(after.isCompletedSem1).toBe(false);
        ...
        expect(Array.isArray(after.teacherCompletions)).toBe(true);
        expect(after.teacherCompletions.length).toBe(0);
        expect(after.teacherCompletionsByYear?.[String(prev._id)]?.length).toBe(1);
        expect(after.completionHistoryByYear?.[String(prev._id)]?.isCompleted).toBe(true);
    });
});
