...
describe('teacher progress N/A cases', () => {
    ...
    it('shows N/A for Polyvalent when template only has Arabic and English items', async () => {
        ...
        const tpl = await GradebookTemplate_1.GradebookTemplate.create({ name: 'tpl', pages: [{ blocks: [{ type: 'language_toggle', props: { items: [{ code: 'ar', label: 'Arabe', active: false }, { code: 'en', label: 'Anglais', active: false }], blockId: 'b1' } }] }], currentVersion: 1 });
    ...
    it('shows N/A for categories when template items are filtered out by level', async () => {
        ...
        const tpl = await GradebookTemplate_1.GradebookTemplate.create({ name: 'tpl2', pages: [{ blocks: [{ type: 'language_toggle', props: { items: [{ code: 'fr', label: 'Français', active: false, level: 'PS' }], blockId: 'b1' } }] }], currentVersion: 1 });
        await TemplateAssignment_1.TemplateAssignment.create({ templateId: String(tpl._id), studentId: String(student._id), completionSchoolYearId: String(active._id), assignedTeachers: [String(t._id)], assignedBy: String(t._id), status: 'completed', isCompleted: true, teacherCompletions: [{ teacherId: String(t._id), completed: true, completedAt: new Date() }] });
        const subToken = (0, auth_1.signToken)({ userId: String(sub._id), role: 'SUBADMIN' });
        const res = await request(app).get('/subadmin-assignments/teacher-progress-detailed').set('Authorization', `Bearer ${subToken}`);
        expect(res.status).toBe(200);
