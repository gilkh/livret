...
studentsRouter.post('/:id/mark-leaving', requireAuth(['ADMIN', 'SUBADMIN']), async (req, res) => {
  const { id } = req.params
  const adminId = (req as any).user.userId
  ...
  try {
    ...
    const assignment = await TemplateAssignment.findOne({ studentId: id }).sort({ assignedAt: -1 }).lean()
...
studentsRouter.post('/:id/undo-leaving', requireAuth(['ADMIN', 'SUBADMIN']), async (req, res) => {
  const { id } = req.params
  const adminId = (req as any).user.userId
  ...
  try {
    ...
    if (leftSchoolYearId) {
      await Enrollment.updateMany(
        { studentId: id, schoolYearId: leftSchoolYearId, status: 'left' },
        { status: 'active', promotionStatus: 'pending' }
      )
    }

    // Delete the "left_school" snapshot if it exists
    if (leftSchoolYearId) {
      await SavedGradebook.deleteOne({
        studentId: id,
...
studentsRouter.get('/left/all', requireAuth(['ADMIN', 'SUBADMIN']), async (req, res) => {
  try {
    const students = await Student.find({ status: 'left' }).lean()
    
    // Get school year names for display
    const yearIds = students.map(s => s.leftSchoolYearId).filter(Boolean)
    const years = await SchoolYear.find({ _id: { $in: yearIds } }).lean()
    const yearMap: Record<string, string> = {}
    for (const y of years) yearMap[String(y._id)] = y.name
...
async function fetchUnassignedStudents(schoolYearId: string) {
  ...
  const yearEnrollments = await Enrollment.find({ schoolYearId, status: { $ne: 'left' } }).lean()
  ...
  const taggedStudents = await Student.find({ schoolYearId, status: { $ne: 'left' } }).lean()
  ...
  if (missingIds.length > 0) {
    ...
    extraStudents = await Student.find({ _id: { $in: missingIds }, status: { $ne: 'left' } }).lean()
  }
