...
describe('Feature Verification Tests', () => {
  ...
  describe('Template upsert safety & Single-assignment POST', () => {
    it('should respect force: false (no reset) and force: true (reset)', async () => {
      const admin = await User.create({ email: 'admin@test.com', role: 'ADMIN', displayName: 'Admin', passwordHash: 'hash' })
      ...
      const student = await Student.create({ firstName: 'Test', lastName: 'Student', dateOfBirth: new Date(), logicalKey: 'TS1' })
      const tpl = await GradebookTemplate.create({ name: 'Tpl1', pages: [], currentVersion: 1 })
      ...
      let assignment = await TemplateAssignment.findOne({ templateId: tpl._id, studentId: student._id }).lean()
      ...
      assignment = await TemplateAssignment.findOne({ templateId: tpl._id, studentId: student._id }).lean()
      ...
      if (res3.status !== 200) console.log('Force update failed:', res3.body)
  ...
  describe('Signature logic improvements', () => {
    it('should support explicit signaturePeriodId and enforce uniqueness', async () => {
      const subAdmin = await User.create({ email: 'sub@test.com', role: 'SUBADMIN', displayName: 'Sub', passwordHash: 'hash' })
      const token = signToken({ userId: String(subAdmin._id), role: 'SUBADMIN' })
      
      const sy = await SchoolYear.create({ name: '2024/2025', active: true, activeSemester: 2, startDate: new Date('2024-09-01'), endDate: new Date('2025-07-01') })
      const student = await Student.create({ firstName: 'Test', lastName: 'Student', dateOfBirth: new Date(), logicalKey: 'TS1', level: 'PS' })
      const tpl = await GradebookTemplate.create({ name: 'Tpl1', pages: [], currentVersion: 1 })
      ...
      await RoleScope.create({ userId: subAdmin._id, levels: ['PS'] })
      
      // Also need Enrollment
      const cls = await ClassModel.create({ name: 'PS A', level: 'PS', schoolYearId: String(sy._id) })
      ...
      if (res1.status !== 200) console.log('Sign error:', res1.body)
      expect(res1.status).toBe(200)
      
      const sig1 = await TemplateSignature.findOne({ templateAssignmentId: assignment._id }).lean()
  ...
  describe('Student promotion atomicity & SavedGradebook', () => {
    it('should promote student, create snapshot, and update enrollment atomically', async () => {
      const subAdmin = await User.create({ email: 'sub2@test.com', role: 'SUBADMIN', displayName: 'Sub2', passwordHash: 'hash' })
      const token = signToken({ userId: String(subAdmin._id), role: 'SUBADMIN' })
      
      const sy = await SchoolYear.create({ name: '2024/2025', active: true, activeSemester: 2, sequence: 1, startDate: new Date('2024-09-01'), endDate: new Date('2025-07-01') })
      const nextSy = await SchoolYear.create({ name: '2025/2026', active: false, sequence: 2, startDate: new Date('2025-09-01'), endDate: new Date('2026-07-01') })
      
      const student = await Student.create({ firstName: 'Promo', lastName: 'Student', dateOfBirth: new Date(), logicalKey: 'PS1', level: 'PS' })
      const tpl = await GradebookTemplate.create({ name: 'TplPromo', pages: [], currentVersion: 1 })
      ...
      const saved = await SavedGradebook.findOne({ studentId: student._id, schoolYearId: sy._id }).lean()
      ...
      const oldEnr = await Enrollment.findOne({ studentId: student._id, schoolYearId: sy._id }).lean()
      expect(oldEnr!.status).toBe('promoted')
      
      const newEnr = await Enrollment.findOne({ studentId: student._id, schoolYearId: nextSy._id }).lean()
      expect(newEnr).toBeDefined()
      expect(newEnr!.status).toBe('active')

      // Verify Assignment has promotion record
      const updatedAss = await TemplateAssignment.findById(assignment._id).lean()
  ...
  describe('Saved gradebooks read behavior', () => {
    it('should return snapshot as-is without patching', async () => {
        const admin = await User.create({ email: 'admin3@test.com', role: 'ADMIN', displayName: 'Admin3', passwordHash: 'hash' })
        const token = signToken({ userId: String(admin._id), role: 'ADMIN' })

        // Create a "broken" saved gradebook (empty assignment data)
        const sg = new SavedGradebook({
            studentId: new mongoose.Types.ObjectId(),
            schoolYearId: new mongoose.Types.ObjectId(),
            level: 'PS',
