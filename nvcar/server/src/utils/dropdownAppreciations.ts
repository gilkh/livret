...
const normalizeStudentSex = (value: unknown): 'female' | 'male' | null => {
  if (value === 'female' || value === 'male') return value
  return null
}

const getAppreciationEntry = (dropdownBlock: any, rawValue: unknown) => {
...
export const resolveDropdownDisplayValue = ({
  ...
  const entry = getAppreciationEntry(dropdownBlock, selected)
  if (!entry) return selected

  const sex = normalizeStudentSex(studentSex)
  if (sex === 'female') return normalizeText(entry?.femaleText) || selected
  if (sex === 'male') return normalizeText(entry?.maleText) || selected
