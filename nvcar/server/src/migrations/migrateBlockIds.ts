...
import { ensureStableBlockIds, ensureStableExpandedTableRowIds, buildBlocksById } from '../utils/templateUtils'
...
interface MigrationStats {
  ...
  dataKeysMigrated: number
  errors: string[]
}

async function migrateTemplates(stats: MigrationStats): Promise<void> {
  console.log('\n=== Migrating Templates ===')
  ...
  for (const template of templates) {
    ...
    if (needsUpdate) {
      try {
        ...
        const fixedPages = ensureStableBlockIds(undefined, pages)
        const fixedPagesWithRowIds = ensureStableExpandedTableRowIds(undefined, fixedPages)
        
        // Update the template
        await GradebookTemplate.findByIdAndUpdate(template._id, {
        ...
        if (Array.isArray(template.versionHistory) && template.versionHistory.length > 0) {
          const updatedHistory = template.versionHistory.map((version: any) => {
            if (Array.isArray(version.pages)) {
              const fixedVersionPages = ensureStableBlockIds(undefined, version.pages)
              const fixedVersionPagesWithRowIds = ensureStableExpandedTableRowIds(undefined, fixedVersionPages)
...
async function migrateAssignmentData(stats: MigrationStats): Promise<void> {
  ...
  for (const assignment of assignments) {
    ...
    if (!assignment.data || typeof assignment.data !== 'object') continue
    ...
    if (needsUpdate) {
      try {
        await TemplateAssignment.findByIdAndUpdate(assignment._id, {
          $set: { data: newData },
          $inc: { dataVersion: 1 }
        })
