    ...
    return (
        <div style={{ padding: 24 }}>
            <div className="card">
                ...
                {saveStatus && <div className="note" style={{ marginTop: 8, color: '#6c5ce7' }}>{saveStatus}</div>}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
                    ...
                    <div className="card">
                        <h3>Édition langue</h3>
                        <div className="note" style={{ marginBottom: 12 }}>Vous pouvez uniquement modifier les drapeaux de langue.</div>
                        {template.pages[selectedPage].blocks.map((b, idx) => {
                            if (b.type !== 'language_toggle') return null
                            return (
                                <div key={idx} style={{ marginBottom: 16, borderTop: '1px solid #ddd', paddingTop: 12 }}>
                                    <div className="note" style={{ fontWeight: 'bold', marginBottom: 8 }}>Bloc langue #{idx + 1}</div>
                                    {(b.props.items || []).map((it: any, i: number) => (
                                        <div key={i} className="competency" style={{ marginBottom: 8 }}>
                                            <div>{it.label || it.code}</div>
                                            <button
                                                className={it.active ? 'btn' : 'btn secondary'}
                                                onClick={() => {
                                                    const newItems = [...(b.props.items || [])]
                                                    newItems[i] = { ...newItems[i], active: !newItems[i].active }
                                                    updateLanguageToggle(selectedPage, idx, newItems)
                                                }}
                                            >
                        ...
                        {template.pages[selectedPage].blocks.filter(b => b.type === 'language_toggle').length === 0 && (
                            <div className="note">Aucun bloc de langue sur cette page.</div>
                        )}
                    </div>
                </div>
            </div>
